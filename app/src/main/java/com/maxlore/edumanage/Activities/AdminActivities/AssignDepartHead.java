...
public class AssignDepartHead extends AppCompatActivity {
    ...
    private String dept_name, dept_id, currentdeparthead;
    private int spinpos;
    ...
    private void assign(int currentPosition) {
        AlertDialog.Builder builder =
                new AlertDialog.Builder(this);
        builder.setTitle("Confirmation");
        message = "Are you Sure  ?";
        for (int i = 0; i < dept_emp_list.size(); i++) {
            if (dept_emp_list.get(i) == (listTeacherArrayList.get(currentPosition).getEmployeeId())) {
    ...
    private void AssignDepartmenthead() {
        try {
            if (UIUtil.isInternetAvailable(this)) {
                ...
                jsonObject.addProperty("department_id", dept_id);
                ...
                RetrofitAPI.getInstance(this).getApi().assigndepartmenthead(jsonObject, new Callback<JsonObject>() {
                    @Override
                    public void success(JsonObject object, Response response) {
                        ...
                        try {
                            if (object == null) {
                                Toast.makeText(AssignDepartHead.this, "Something went wrong, try after sometime...", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            if (object.get("status").getAsInt() == Constants.SUCCESS) {
                                Toast.makeText(AssignDepartHead.this, object.get("message").getAsString(), Toast.LENGTH_SHORT).show();
                            }
